...
class Entity:
    ...
    def hitbox_tiles(self):
        left = int(self.position.x // GameSettings.TILE_SIZE)
        right = int((self.position.x + GameSettings.TILE_SIZE - 1) // GameSettings.TILE_SIZE)
        top = int(self.position.y // GameSettings.TILE_SIZE)
        bottom = int((self.position.y + GameSettings.TILE_SIZE - 1) // GameSettings.TILE_SIZE)
        return [(tx, ty) for tx in range(left, right + 1) for ty in range(top, bottom + 1)]